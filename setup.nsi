...
; -----------------------------------------
!ifndef VERSION
  !getdllversion "${BUILD_DIR}\${EXE_NAME}" expv_
...
VIAddVersionKey /LANG=1033 "LegalCopyright" "Copyright © 2026 ${PUBLISHER}. All rights reserved."
...
Section "Install"
  ...
  ; 기존 공개 인증서(.cer) 설치된 거 삭제 - ExecWait 만 쓰면 창 뜸. 그래서 nsExec + Pop 조합으로.
...
Section "Uninstall"
  ...
  ; 사용자데이터 삭제 여부 확인(삭제한다고 하면 지우기 - %LOCALAPPDATA% 확인)
  MessageBox MB_YESNO|MB_ICONQUESTION "사용자 데이터를 모두 삭제하시겠습니까?" IDNO skip_appdata
    DetailPrint "사용자 데이터 삭제 중..."
    RMDir /r "$LOCALAPPDATA\${ORG_NAME}\${APP_NAME}"
    RMDir "$LOCALAPPDATA\${ORG_NAME}"
    DeleteRegKey HKCU "${SOFT_KEY}"
    DeleteRegKey /ifempty HKCU "${GROUP_KEY}"
    DeleteRegKey HKLM "${SOFT_KEY}"
    DeleteRegKey /ifempty HKLM "${GROUP_KEY}"
  skip_appdata:

  ; 인증서 제거하고 설치폴더 제거하고 - ExecWait 만 쓰면 창 뜸. 그래서 nsExec + Pop 조합으로.
  nsExec::Exec 'certutil -delstore "Root" "${CERT_CODE}"'
  Pop $0
  RMDir /r "$INSTDIR"
